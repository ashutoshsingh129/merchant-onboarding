...
// Cyprus-specific configuration
const DIRECTOR_EXECUTIVE_COUNTRIES: string[] = ['CY']; // Cyprus requires directors/executives
const IBAN_COUNTRIES: string[] = ['CY']; // Cyprus uses IBAN
const DEFAULT_COUNTRY = 'CY';

// DirectOnboardFormData and DirectOnboardFormProps are imported from types.ts

const CyprusForm: React.FC<DirectOnboardFormProps> = ({
    accountId,
    email,
    businessType,
    country,
                        ...
                        {isUS ? (
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <TextField
                                    ...
                                    placeholder="e.g., Cypriot ID number"
    ...
    const handleInputChange = (field: keyof DirectOnboardFormData, value: any) => {
        setFormData(prev => {
            ...
            // Normalize IBAN for IBAN countries (SE, FR, CY): strip spaces and uppercase
            if (
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Validate director requirement for Cyprus
            if (shouldShowDirectorsExecutives) {
            ...
            // Handle representative SSN (for all business types in Cyprus - individual, company, non-profit, and government_entity)
            if (
                                    ...
                                    <Grid size={{ xs: 12 }}>
                                        {formData.individual_address_country === 'US' ? (
                                            ...
                                            <TextField
                                                ...
                                                placeholder="e.g., Cyprus ID card number"
                                                helperText="Provide full national ID number (Cyprus ID card number)"
                                            />
                                        )}
                                    </Grid>
                                </>
