/**
 * France-specific handler for direct onboarding
 */
...
class FranceHandler extends BaseHandler {
  constructor() {
    super('FR');
  ...
  /**
   * Handle company business type for France
   */
  async handleCompany(accountUpdateData, reqBody, stripe, accountId) {
    ...
    // Handle additional directors (France requires directors)
    if (Array.isArray(directors) && directors.length > 0) {
      await this.createDirectors(stripe, accountId, directors);
      accountUpdateData.company.directors_provided = true;
    }

    // Handle additional executives (France requires executives)
  ...
  /**
   * Override external account handling to use EUR currency for France
   */
  async handleExternalAccount(stripe, accountId, object, country, currency, reqBody) {
    // Force EUR currency for France
    return await super.handleExternalAccount(stripe, accountId, object, country, 'eur', reqBody);
  }
}

module.exports = FranceHandler;
